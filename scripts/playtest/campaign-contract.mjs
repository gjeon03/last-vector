...
await report.check({
  id: 'MISSION.campaign-view-standalone',
  name: 'Catalog-driven campaign projection exposes the standalone chapter rail',
  assertion: 'Empty progress locks DEAD SIGNAL and a CAIRN clear authorises it as the next mission.',
}, () => {
  ...
  verify(JSON.stringify(empty) === JSON.stringify(expected),
    'The catalog projection changed the standalone campaign snapshot.', { empty, expected });
  ...
  verify(cleared.nextMissionId === 'dead-signal'
    ...
    && cleared.missions[1]?.id === 'dead-signal'
    && cleared.missions[1]?.state === 'available'
    && cleared.missions[1]?.chapter === 3
    && cleared.missions[1]?.capabilities[0] === 'fire'
    && cleared.missions[1]?.mastery[0]?.id === 'all-nodes'
    && cleared.missions[1]?.mastery[1]?.id === 'accuracy',
  'Cleared CAIRN progress did not authorise the DEAD SIGNAL view.', cleared);
