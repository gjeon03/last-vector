...
async function runInputProof({ report, session, options }) {
  ...
  await report.check({
    id: 'INPUT.last-ascent-lmb-noop',
    name: 'LAST ASCENT keeps Shift boost while LMB remains inactive',
    assertion: 'On LAST ASCENT, pointer-locked LMB neither fires nor boosts; Shift remains sole boost.',
  }, async () => {
    ...
    const route = new URL(await callHarness(page, 'routeUrl', ['last-ascent']));
    ...
    verify(response?.ok(), 'LAST ASCENT canonical route did not reload successfully.', {
    ...
    await mouse('mouseup', 0);
    verify(lmb.fire === false && lmb.boost === false,
      'LAST ASCENT accepted an inactive LMB action.', { lmb, shift });
    verify(shift.fire === false && shift.boost === true,
      'Shift did not remain the sole LAST ASCENT boost binding.', { lmb, shift });
    return { route: route.href, lmb, shift };
  });
