...
assert.equal(debug.activeSources, 5);
...
assert.ok(debug.colliders > 0 && debug.colliders <= 45);

for (const collider of field.colliders) {
  for (const source of state.sources) {
    const surfaceClearance = collider.position.distanceTo(source.position) - collider.radius;
    assert.ok(
      surfaceClearance >= 500 - 1e-6,
      `${collider.id} violates ${source.id} source clearance: ${surfaceClearance}`,
    );
  }
}
...
const instanceMatrices = sourceMeshes.map((mesh) => Array.from(mesh.instanceMatrix.array));
...
for (let index = 0; index < sourceMeshes.length; index++) {
  assert.deepEqual(Array.from(sourceMeshes[index].instanceMatrix.array), instanceMatrices[index]);
}
...
for (let index = 1; index < VALIDATED_RELAY_HARVEST_LAYOUTS.length; index++) {
  ...
  const layoutField = new RelayHarvestField({ sources: layoutState.sources, lighting });
  for (const collider of layoutField.colliders) {
    for (const source of layoutState.sources) {
      assert.ok(
        collider.position.distanceTo(source.position) - collider.radius >= 500 - 1e-6,
        `${layoutState.layoutSignature}: ${collider.id} violates ${source.id} clearance`,
      );
    }
  }
  layoutField.dispose();
